...
BEGIN
    ...
    IF p_cliente_id <> (SELECT cliente_id FROM clientes WHERE cliente_id = p_cliente_id) THEN 
        SET mensaje :='Cliente inexistente';
    END IF;

    IF p_detalle <> (SELECT detalle_venta_id FROM detalle_venta WHERE detalle_venta WHERE detalle_venta_id = p_detalle) THEN
        SET mensaje := 'Detalle de venta inexistente';
    END IF;

    INSERT INTO ventas(cliente_id, detalle_venta_id, precio_final)
        VALUES
            (p_cliente_id, p_detalle, p_precio);
        SET mensaje:= 'Venta creada';
